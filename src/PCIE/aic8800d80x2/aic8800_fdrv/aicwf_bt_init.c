...
int aicbt_ext_patch_data_load(struct rwnx_hw *rwnx_hw,
	...
	if (ext_patch_nb > 0) {
		...
		for (index = 0; index < patch_info->ext_patch_nb; index++) {
			...
			sprintf(ext_patch_file_name, "%s%d.bin",
				bt_patch_name[rwnx_hw->pcidev->chip_id -
			...
			if (rwnx_plat_bin_fw_upload_2(rwnx_hw, addr,
						      ext_patch_file_name)) {
				ret = -1;
				break;
			}
...
int aicwf_bt_init(struct rwnx_hw *rwnx_hw)
{
	...
	aicwf_pcie_tran(rwnx_hw->pcidev, (void *)BT_INIT_DONE_ADDR, &bt_init_st,
	...
	if ((bt_init_st & BT_INIT_DONE_BIT) == 0) {
		...
		AICWFDBG(LOGINFO, "%s chip_id:%d\r\n", __func__,
			 rwnx_hw->pcidev->chip_id);
		bt_patch_file = &bt_patch_name[rwnx_hw->pcidev->chip_id -
					       PRODUCT_ID_AIC8800D80];

		head = aicbt_patch_table_alloc(rwnx_hw,
					       bt_patch_file->fw_patch_table);
		if (head == NULL) {
		...
		while ((bt_init_st & BT_INIT_DONE_BIT) == 0) {
			err = aicwf_pcie_tran(rwnx_hw->pcidev,
					      (void *)BT_INIT_DONE_ADDR,
					      &bt_init_st, 4, AIC_TRAN_EMB2DRV,
					      1);
			cnt++;
		...
		AICWFDBG(
			LOGINFO,
			"bt init done: err = %d, cnt = %d, st = %x, use %lld ns\n",
