...
int aicwf_bus_init(uint bus_hdrlen, struct device *dev)
{
	...
	bus_if->busrx_thread = kthread_run(
		pcie_rxbuf_rep_thread, (void *)bus_if->bus_priv.pci->rx_priv,
		"pcie_rxbuf_rep_thread");
#ifdef CONFIG_RX_SKBLIST
#ifndef CONFIG_RX_TASKLET
	bus_if->rx_thread = kthread_run(pcie_rxbuf_process_thread,
					(void *)bus_if->bus_priv.pci->rx_priv,
					"pcie_rxbuf_process_thread");
#endif
#endif
#endif
