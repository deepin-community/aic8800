...
static int rwnx_close(struct net_device *dev)
{
		...
		if (usbdev->bus_if->state != BUS_DOWN_ST)
...

			{
				...
				if (testmode == 0) {
					...
					rwnx_send_me_chan_config_req(
						rwnx_hw, country_code);
	...
	ret = aicwf_sdio_writeb(g_rwnx_plat->sdiodev,
				g_rwnx_plat->sdiodev->sdio_reg.wakeup_reg, 2);
				      ...
				      struct vif_params *params)
{
	...
	RWNX_DBG(RWNX_FN_ENTRY_STR);
	AICWFDBG(LOGINFO, "change_if: %d to %d, %d, %d", vif->wdev.iftype, type,
				    ...
				    u16 reason_code)
{
	...
	if (atomic_read(&rwnx_vif->drv_conn_state) ==
		...
		AICWFDBG(LOGINFO,
			 "%s call cfg80211_connect_result reason:%d \r\n",
...
enum { PHYMODE_B, PHYMODE_G, PHYMODE_A, PHYMODE_N, PHYMODE_AC, PHYMODE_AX };
...
static const u32 he_mcs_map_to_rate[2][3][3][12] = {
	//NSS/GI/BW/MCS
	{ //1ss
	  {
		  //0.8us
		  { 8600, 17200, 25800, 34400, 51600, 68800, 77400, 86000,
		    103200, 114700, 129000, 143400 },
		  { 17200, 34400, 51600, 68800, 103200, 137600, 154900, 172100,
		    206500, 229400, 258100, 286800 },
		  { 36000, 72100, 108100, 144100, 216200, 288200, 324300,
		    360300, 432400, 480400, 540400, 600400 },
	  },
	  {
		  //1.6us
		  { 8100, 16300, 24400, 32500, 48800, 65000, 73100, 81300,
		    97500, 108300, 121900, 135400 }, //20M,
		  { 16300, 32500, 48800, 65000, 97500, 130000, 146300, 162500,
		    195000, 216700, 243800, 270800 }, //40M,
		  { 34000, 68100, 102100, 136100, 204200, 272200, 306300,
		    340300, 408300, 453700, 510400, 567100 }, //80M,
	  },
	  {
		  //3.2us
		  { 7300, 14600, 21900, 29300, 43900, 58500, 65800, 73100,
		    87800, 97500, 109700, 121900 },
		  { 14600, 29300, 43900, 58500, 87800, 117000, 131600, 146300,
		    175500, 195000, 219400, 243800 },
		  { 30600, 61300, 91900, 122500, 183800, 245000, 275600, 306300,
		    367500, 408300, 459400, 510400 },
	  } },
	{ //2ss
	  {
		  //0.8us
		  { 17200, 34400, 51600, 68800, 103200, 137600, 154900, 172100,
		    206500, 229400, 258100, 286800 }, //242tone
		  { 34400, 68800, 103200, 137600, 206500, 275300, 309700,
		    344100, 412900, 458800, 516200, 573500 }, //484tone
		  { 72100, 144100, 216200, 288200, 432400, 576500, 648500,
		    720600, 864700, 960700, 1080900, 1201000 }, //996tone
	  },
	  {
		  // 1.6us
		  { 16300, 32500, 48800, 65000, 97500, 130000, 146300, 162500,
		    195000, 216700, 243800, 270800 },
		  { 32500, 65000, 97500, 130000, 195000, 260000, 292500, 325000,
		    390000, 433300, 487500, 541700 },
		  { 68100, 136100, 204200, 272200, 408300, 544400, 612500,
		    680600, 816700, 907400, 1020800, 1134200 },
	  },
	  { // 3.2us
	    { 14600, 29300, 43900, 58500, 87800, 117000, 131600, 146300, 175500,
	      195000, 219400, 243800 },
	    { 29300, 58500, 87800, 117000, 175500, 234000, 263300, 292500,
	      351000, 390000, 438800, 487500 },
	    { 61300, 122500, 183800, 245000, 367500, 490000, 551300, 612500,
	      735000, 816600, 918800, 1020800 } } }
};

static const u32 vht_mcs_map_to_rate[2][2][3][10] = {
	//NSS//GI/BW/MCS
	{ // 1ss
	  {
		  //LONG GI
	...
	{ //2ss
	  ...
	  },
	  {
		  ...
		  { 30000, 60000, 90000, 120000, 180000, 240000, 270000, 300000,
		    360000, 400000 }, //40M
		  { 65000, 130000, 195000, 260000, 390000, 520000, 585000,
		    650000, 780000, 866700 }, //80M
	  } }
};

static const u32 ht_mcs_map_to_rate[2][2][3][8] = {
	//NSS//GI/BW/MCS
	{ // 1SS
	  {
		  //LONG GI
		  { 6500, 13000, 19500, 26000, 39000, 52000, 58500,
		    65000 }, //20M
		  { 13500, 27000, 40500, 54000, 81000, 108000, 121500,
		    135000 }, //40M
	  },
	  {
		  //SHORT GI
	...
	{ // 2SS
	  {
		  //LONG GI
		  { 13000, 26000, 39000, 52000, 78000, 104000, 117000,
		    130000 }, //20M
		  { 27000, 54000, 81000, 108000, 162000, 216000, 243000,
		    270000 }, //40M
	  },
	  {
		  //SHORT GI
		  { 14400, 28900, 43300, 57800, 86700, 115600, 130000,
		    144400 }, //20M
		  { 30000, 60000, 90000, 120000, 180000, 240000, 270000,
		    300000 }, //40M
	  }
...
int rwnx_fill_station_info(struct rwnx_sta *sta, struct rwnx_vif *vif,
			   struct station_info *sinfo, u8 *phymode,
			   u32 *tx_phyrate, u32 *rx_phyrate)
{
	...
	case FORMATMOD_HT_GF:
		...
		tx_phyrate_local =
			ht_mcs_map_to_rate[sinfo->txrate.nss - 1]
					  [rate_info->giAndPreTypeTx]
					  [rate_info->bwTx][sinfo->txrate.mcs];
		break;
	case FORMATMOD_VHT:
		...
		tx_phyrate_local =
			vht_mcs_map_to_rate[sinfo->txrate.nss - 1]
					   [rate_info->giAndPreTypeTx]
					   [rate_info->bwTx][sinfo->txrate.mcs];
	...
	case FORMATMOD_HE_ER:
		...
		sinfo->txrate.nss = ((rate_info->mcsIndexTx >> 4) & 0x7) + 1;
		tx_phyrate_local =
			he_mcs_map_to_rate[sinfo->txrate.nss - 1]
					  [rate_info->giAndPreTypeTx]
					  [rate_info->bwTx][sinfo->txrate.mcs];
	...
	case FORMATMOD_HT_GF:
		...
		if (rx_vect1->ht.mcs == 32) { //mcs 32
			...
			rx_phyrate_local =
				ht_mcs_map_to_rate[rx_vect1->ht.num_extn_ss]
						  [rx_vect1->ht.short_gi]
						  [rx_vect1->ch_bw][mcs];
		}
		break;
	case FORMATMOD_VHT:
		...
		rx_phyrate_local =
			vht_mcs_map_to_rate[rx_vect1->vht.nss]
					   [rx_vect1->vht.short_gi]
					   [rx_vect1->ch_bw][sinfo->rxrate.mcs];
	...
	case FORMATMOD_HE_ER:
		...
		rx_phyrate_local =
			he_mcs_map_to_rate[rx_vect1->he.nss]
					  [rx_vect1->he.gi_type]
					  [rx_vect1->ch_bw][sinfo->rxrate.mcs];
		break;
#else
				     ...
				     struct station_info *sinfo)
{
	...
	if (sta)
		return rwnx_fill_station_info(sta, vif, sinfo, NULL, NULL,
					      NULL);
...
int btdm_log(struct rwnx_hw *rwnx_hw)
{
	...
	for (i = 0; i < btdm_log_size; i += size) {
		...
		err = rwnx_send_dbg_mem_block_read_req(
			rwnx_hw, btdm_log_addr + i, size, &cfm);
		if (!err) {
			...
			while ((newline_pos = memchr(buffer, '\n', buf_len)) !=
				...
				remaining_len =
					buf_len - (newline_pos - buffer + 1);
				if (newline_pos < buffer + buf_len - 1) {
					memmove(buffer, newline_pos + 1,
						remaining_len);
				} else {
...
int rwnx_cfg80211_init(struct rwnx_plat *rwnx_plat, void **platform_data)
{
...
#endif
	memset(&rwnx_hw->all_restore_param, 0,
	...
	tasklet_init(&rwnx_hw->task_txrestart, rwnx_txrestart_task,
		     (unsigned long)rwnx_hw);
#ifdef CONFIG_RX_TASKLET
	tasklet_init(&rwnx_hw->task_rx_process, rwnx_task_rx_process,
		     (unsigned long)rwnx_hw);
#endif
...
#else
	ret = rwnx_send_set_stack_start_req(rwnx_hw, 1, feature.hwinfo < 0,
					    feature.hwinfo, 0, &set_start_cfm);
#endif
	...
	memcpy(rwnx_hw->fw_version, fw_version.fw_version,
	       fw_version.fw_version_len > 63 ? 63 : fw_version.fw_version_len);
...
#endif
			if (!set_pcie_down)
				rwnx_send_set_stack_start_req(
					rwnx_hw, 0, 0, 0, 0, &set_start_cfm);
