					   ...
					   char *argv[], char *command)
{
	...
	if ((dev->chip_id == PRODUCT_ID_AIC8800DC) ||
		...
		res_len = (3 * 3 + 3 * 6) * 2;
		   ...
		   PRODUCT_ID_AIC8800D80X2) { // 3 * 2 (2.4g) *2 + 3 * 6 (5g) *2
		res_len = (3 * 3 + 3 * 6) * 2 * 2;
	} else { // 7 = 3(2.4g) + 4(5g)
		res_len = 3 + 4;
	}
...
static int aic_priv_cmd_exec_flash_oper(struct rwnx_hw *rwnx_hw, int argc,
					char *argv[], char *command)
{
	u8_l func = 0;
	if (argc > 1) {
		...
		rwnx_send_rftest_req(rwnx_hw, EXEC_FLASH_OPER, sizeof(func),
				     &func, &cfm);
...
void aicwf_pcie_dump(struct rwnx_hw *rwnx_hw)
{
	...
	for (i = 0; i < IPC_TXDMA_DESC_CNT; i++) {
		AICWFDBG(
			LOGERROR, "tx[%d] desc %x, dma %x, ready %x, pt %x\n",
			i, rwnx_hw->ipc_env->shared->txdesc_addr[i],
			rwnx_hw->ipc_env->shared->txdesc[i].api.host.packet_addr,
	...
	AICWFDBG(LOGERROR, "fc %d txdata cnt %d push %d reserved %d total %d\n",
		 rwnx_hw->fc, atomic_read(&rwnx_hw->txdata_cnt),
		 atomic_read(&rwnx_hw->txdata_cnt_push),
		 atomic_read(&rwnx_hw->txdata_reserved),
		 atomic_read(&rwnx_hw->txdata_total));
	...
	AICWFDBG(LOGERROR, "driver txc/rxc=%d,%d, fw txc/rxc=%d,%d\n",
		 ...
		 *(volatile uint32_t *)&rwnx_hw->ipc_env->shared->txc_cnt,
		 *(volatile uint32_t *)&rwnx_hw->ipc_env->shared->rxc_cnt);
	//aic debug end
}
...
int get_cs_info(struct rwnx_vif *vif, u8 *mac_addr, u8 *val)
{
	...
	if (RWNX_VIF_TYPE(vif) == NL80211_IFTYPE_MONITOR)
		...
		list_for_each_entry(sta_iter, &vif->ap.sta_list, list) {
			if (sta_iter->valid &&
			    ether_addr_equal(sta_iter->mac_addr, mac_addr)) {
				sta = sta_iter;
				break;
			}
		}
	...
	if (sta) {
		...
		rwnx_fill_station_info(sta, vif, &sinfo, &phymode, &tx_phyrate,
				       &rx_phyrate);
		...
		cs_info.snr =
			(int8_t)(rx_vect2->evm1) + (int8_t)(rx_vect2->evm2) / 2;
		...
		AICWFDBG(
			LOGINFO,
			"phymode=%d. bw=%d, rssi=%d, tx_phyrate=%d, rx_phyrate=%d\n",
...
#define PRIVATE_COMMAND_DEF_LEN 4096
	...
	if (!strncasecmp(command, CMD_SET_COUNTRY, strlen(CMD_SET_COUNTRY)) &&
		...
		ret = 0;
		goto exit;
	} else if (!strncasecmp(command, CMD_GET_CS_INFO,
				strlen(CMD_GET_CS_INFO))) {
		u8 mac_addr[6];
		u8 val[128];
		int i;
		u8 *offset;
		...
		goto exit;
	} else if (!strncasecmp(command, CMD_SET_PCIE_DOWN,
				strlen(CMD_SET_PCIE_DOWN))) {
