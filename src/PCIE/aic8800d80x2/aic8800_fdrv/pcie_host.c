...
void aicwf_pcie_host_init(struct ipc_host_env_tag *env, void *cb,
			  ...
			  void *pthis)
{
	...
	env->unsuprxvec_sz = max(sizeof(struct rx_vector_desc),
...
int pcie_host_msg_push(struct ipc_host_env_tag *env, void *msg_buf,
		       uint16_t len)
{
	int i;
		      ...
		      struct sk_buff *skb, int hw_queue)
{
	...
	sw_txhdr->ipc_desc.dma_addr =
		sw_txhdr->ipc_data
			.dma_addr; //txdesc_host->api.host.packet_addr[0];
	sw_txhdr->ipc_desc.size =
		sw_txhdr->frame_len; //txdesc_host->api.host.packet_len[0];
	sw_txhdr->idx = dma_idx;

	//printk("skb dma addr=%lx, len=%d\n", sw_txhdr->ipc_data.dma_addr, sw_txhdr->ipc_data.size);
	if (txdesc_host->api.host.ethertype == 0x8e88 ||
	    txdesc_host->api.host.ethertype == 0x888e) {
	...
	dma_sync_single_for_device(rwnx_hw->dev, ipc_hostdesc_buf->dma_addr,
				   sizeof(struct txdesc_host), DMA_TO_DEVICE);
...
void aicwf_pcie_host_txdesc_push(struct ipc_host_env_tag *env,
...
void aicwf_pcie_host_tx_cfm_handler(struct ipc_host_env_tag *env, u32 *data,
				    u8 free)
{
	...
	//while (1)
	{
		...
		uint64_t host_id =
			env->tx_host_id[queue_idx][used_idx % PCIE_TXDESC_CNT];
		...
		AICWFDBG(LOGINFO, "pcie cfm: data：%x, used_idx=%d, skb=%p\n",
			 data[0], used_idx,
			 (struct sk_buff *)(uint64_t)host_id);
#if 1
		...
		if (host_id == 0) {
			...
			AICWFDBG(LOGERROR,
				 "pcie ERROR:No more confirmations\r\n");
			//break;
		}
		// set the cfm status
		...
		AICWFDBG(
			LOGINFO,
			"pcie host_tx_cfm_handler:used_idx=%d, 0x%p, status=%x\r\n",
