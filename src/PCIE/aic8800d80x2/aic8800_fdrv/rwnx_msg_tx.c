			 ...
			 int reqcfm, lmac_msg_id_t reqid, void *cfm)
{
	...
	if ((rwnx_hw->pcidev->bus_if->state == BUS_DOWN_ST) ||
	    (rwnx_hw->pci_suspending && reqid != MM_SET_RESUME_RESTORE_CFM)) {
		rwnx_msg_free(rwnx_hw, msg_params);
...
#endif
#ifdef AICWF_PCIE_SUPPORT
		aicwf_set_cmd_tx((void *)(rwnx_hw->pcidev), cmd->a2e_msg,
				 sizeof(struct lmac_msg) +
					 cmd->a2e_msg->param_len);
#endif
	}
...
int rwnx_send_start(struct rwnx_hw *rwnx_hw)
{
	...
	rwnx_prepare_resume_restore_req(rwnx_hw, MM_START_REQ, start_req_param,
					sizeof(struct mm_start_req));
		       ...
		       u8_l enable_nullcts)
{
	...
	rwnx_prepare_resume_restore_req(rwnx_hw, MM_SET_COEX_REQ, coex_req,
					sizeof(struct mm_set_coex_req));
			   ...
			   struct mm_set_rf_calib_cfm *cfm)
{
	...
	rwnx_prepare_resume_restore_req(rwnx_hw, MM_SET_RF_CALIB_REQ,
					rf_calib_req,
					sizeof(struct mm_set_rf_calib_req));
				  ...
				  struct mm_set_stack_start_cfm *cfm)
{
	...
	rwnx_prepare_resume_restore_req(rwnx_hw, MM_SET_STACK_START_REQ, req,
					sizeof(struct mm_set_stack_start_req));
				  ...
				  int32_t *param, int32_t *param_out)
{
	...
	case ACS_TXOP_REQ:
		...
		AICWFDBG(LOGINFO,
			 "set_acs_txop_req: be: %x,bk: %x,vi: %x,vo: %x\n",
	...
	case STBC_MCS_SET_REQ: {
		...
		req = rwnx_msg_zalloc(MM_SET_VENDOR_HWCONFIG_REQ, TASK_MM,
				      DRV_TASK_ID,
				      sizeof(struct mm_get_stbc_msc_req));
		if (!req)
			return -ENOMEM;
		req->hwconfig_id = hwconfig_id;
	...
	case MAX_AGG_TX_CNT_REQ: {
		...
		if ((u8)param[0] > 1 || (u8)param[1] > 11) {
			AICWFDBG(LOGERROR,
				 "hwvendor max_agg_cnt param error:%x,%x\n",
				 (u8)param[0], (u8)param[1]);
			break;
		}
		req = rwnx_msg_zalloc(MM_SET_VENDOR_HWCONFIG_REQ, TASK_MM,
				      DRV_TASK_ID,
				      sizeof(struct mm_set_max_tx_agg_cnt_req));
		if (!req)
			return -ENOMEM;
		...
		AICWFDBG(
			LOGINFO,
			"hwvendor max_aggr_cnt:en=%d, mcs_thresh=%d, 0x%x, 0x%x, 0x%x, 0x%x\n",
			req->enale, req->mcs_thresh, req->max_agg_cnt[0],
			req->max_agg_cnt[1], req->max_agg_cnt[2],
			req->max_agg_cnt[3]);
	} break;
	case MAX_BW_MCS_THRESH_SET_REQ: {
		...
		if ((u8)param[0] > 1 || (u8)param[1] > 11) {
			AICWFDBG(LOGERROR,
				 "hwvendor max_bw_mcs param error:%x,%x\n",
				 (u8)param[0], (u8)param[1]);
			break;
		}
		req = rwnx_msg_zalloc(
			MM_SET_VENDOR_HWCONFIG_REQ, TASK_MM, DRV_TASK_ID,
			sizeof(struct mm_set_max_bw_mcs_thresh_req));
		if (!req)
			return -ENOMEM;
	...
	case DCM_FORCE_EN_REQ: {
		...
		req = rwnx_msg_zalloc(MM_SET_VENDOR_HWCONFIG_REQ, TASK_MM,
				      DRV_TASK_ID,
				      sizeof(struct mm_set_dcm_force_en_req));
		if (!req)
			return -ENOMEM;
	...
	case AUTO_CCA_EN_REQ: {
		...
		if ((u8)param[0] > 1 || (int8_t)param[1] > 0 ||
		    (u8)param[2] > 1 || (int8_t)param[3] > 0) {
			AICWFDBG(
				LOGERROR,
				"hwvendor auto_cca_en param error:%x,%d,%x,%d\n",
				(u8)param[0], (int8_t)param[1], (u8)param[2],
				(int8_t)param[3]);
			break;
		}
		req = rwnx_msg_zalloc(MM_SET_VENDOR_HWCONFIG_REQ, TASK_MM,
				      DRV_TASK_ID,
				      sizeof(struct mm_set_auto_cca_en_req));
		if (!req)
			return -ENOMEM;
		...
		AICWFDBG(
			LOGINFO,
			"set auto cca en: enable=%d, thresh=%d, default_set=%d, default_thresh=%d\n",
			(u8)param[0], (int8_t)param[1], (u8)param[2],
			(int8_t)param[3]);
	} break;
	case NSS_1T2R_REQ: {
		...
		req = rwnx_msg_zalloc(MM_SET_VENDOR_HWCONFIG_REQ, TASK_MM,
				      DRV_TASK_ID,
				      sizeof(struct mm_set_nss_1t2r_req));
		if (!req)
			return -ENOMEM;
	...
	case ON_AIR_DUTY_CYCLE_REQ: {
		...
		if ((u8)param[1] < 1 || (u8)param[1] > 100) {
			AICWFDBG(
				LOGERROR,
				"hwvendor on_air_duty_cycle param error:%x,%x\n",
				(u8)param[0], (u8)param[1]);
			break;
		}
		req = rwnx_msg_zalloc(
			MM_SET_VENDOR_HWCONFIG_REQ, TASK_MM, DRV_TASK_ID,
			sizeof(struct mm_set_on_air_duty_cycle_req));
		if (!req)
			return -ENOMEM;
		...
		AICWFDBG(LOGINFO,
			 "set on_air duty cycle: enable=%d, percent=%d\n",
			 (u8)param[0], (u8)param[1]);
	} break;
	default:
		return -ENOMEM;
				  ...
				  int32_t *param_in, int32_t *param_out)
{
	...
	case BCN_CFG_REQ:
		...
		rwnx_prepare_resume_restore_req(
			rwnx_hw, MM_SET_VENDOR_SWCONFIG_REQ, req,
			sizeof(struct mm_set_vendor_swconfig_req));
	...
	case EXT_FLAGS_MASK_SET_REQ:
		...
		AICWFDBG(LOGINFO,
			 "ext_flags_mask_set_req: flags mask=0x%x, val=0x%x\n",
			 req->ext_flags_mask_set_req.user_flags_mask,
...
int rwnx_send_txpwr_lvl_v3_req(struct rwnx_hw *rwnx_hw)
{
	...
	if (txpwr_lvl_v3->enable == 0) {
		...
		rwnx_prepare_resume_restore_req(
			rwnx_hw, MM_SET_TXPWR_IDX_LVL_REQ, txpwr_lvl_req,
			sizeof(struct mm_set_txpwr_lvl_req));
...
int rwnx_send_txpwr_lvl_adj_req(struct rwnx_hw *rwnx_hw)
{
	...
	if (txpwr_lvl_adj->enable == 0) {
		...
		rwnx_prepare_resume_restore_req(
			rwnx_hw, MM_SET_TXPWR_LVL_ADJ_REQ, txpwr_lvl_adj_req,
			sizeof(struct mm_set_txpwr_lvl_adj_req));
...
int rwnx_send_txpwr_ofst2x_req(struct rwnx_hw *rwnx_hw)
{
	...
	if (txpwr_ofst2x->enable) {
		...
		rwnx_prepare_resume_restore_req(
			rwnx_hw, MM_SET_TXPWR_OFST_REQ, txpwr_ofst_req,
			sizeof(struct mm_set_txpwr_ofst_req));
...
//}
	...
	req->he_ul_on = false;
		...
		rwnx_prepare_resume_restore_req(rwnx_hw, ME_CONFIG_REQ, req,
						sizeof(struct me_config_req));
	...
	int rwnx_send_me_chan_config_req(struct rwnx_hw * rwnx_hw, char *ccode)
	{
		...
		rwnx_prepare_resume_restore_req(
			rwnx_hw, ME_CHAN_CONFIG_REQ, req,
			sizeof(struct me_chan_config_req));
...
if (sta->he) {
	...
	for (i = 0; i < ARRAY_SIZE(he_capa->mac_cap_info); i++) {
		req->he_cap.mac_cap_info[i] =
			sta->he_cap_elem
				.mac_cap_info[i]; //he_capa->mac_cap_info[i];
	}
	for (i = 0; i < ARRAY_SIZE(he_capa->phy_cap_info); i++) {
		req->he_cap.phy_cap_info[i] =
			sta->he_cap_elem
				.phy_cap_info[i]; //he_capa->phy_cap_info[i];
	}

	req->he_cap.mcs_supp.rx_mcs_80 =
		sta->he_mcs_nss_supp.rx_mcs_80; //mcs_nss_supp->rx_mcs_80;
	req->he_cap.mcs_supp.tx_mcs_80 =
		sta->he_mcs_nss_supp.tx_mcs_80; //mcs_nss_supp->tx_mcs_80;
	req->he_cap.mcs_supp.rx_mcs_160 = mcs_nss_supp->rx_mcs_160;
	req->he_cap.mcs_supp.tx_mcs_160 = mcs_nss_supp->tx_mcs_160;
	req->he_cap.mcs_supp.rx_mcs_80p80 = mcs_nss_supp->rx_mcs_80p80;
	req->he_cap.mcs_supp.tx_mcs_80p80 = mcs_nss_supp->tx_mcs_80p80;
				     ...
				     struct sm_connect_cfm * cfm)
	{
		...
		AICWFDBG(
			...
			__func__, rwnx_vif->drv_vif_index, sme->ssid,
			(int)sme->ssid_len, req->chan.freq, req->auth_type);
...
u8_l vendor_extension_data[]={
				...
				struct cfg80211_scan_request * param)
	{
		...
		req->add_ies = rwnx_hw->scan_ie.dma_addr;
#else
			ie_req = rwnx_msg_zalloc(
				SCANU_VENDOR_IE_REQ, TASK_SCANU, DRV_TASK_ID,
			...
			err = rwnx_send_msg(rwnx_hw, ie_req, 1,
 ...
 *********************************************************************/
	...
	int rwnx_send_resume_restore(struct rwnx_hw * rwnx_hw, u8_l on,
				     u8_l efuse_valid, u8_l set_vendor_info,
				     u8_l fwtrace_redir_en,
				     struct rwnx_vif * vif)
	{
...
//	struct ieee80211_sta_he_cap const *he_cap = NULL;
	...
	struct ieee80211_sta_he_cap const *he_cap;
		...
		all_param_req = rwnx_msg_zalloc(
			MM_SET_RESUME_RESTORE_REQ, TASK_MM, DRV_TASK_ID,
	...
	all_param_req->start_req.lp_clk_accuracy = (u16_l)rwnx_hw->mod_params->lp_clk_ppm;
#else
		memcpy(all_param_req, &rwnx_hw->all_restore_param,
		       sizeof(struct mm_all_restore_param));
#endif

		//*******************add_if
		if (vif != NULL) {
			memcpy(&(all_param_req->add_if_req.addr.array[0]),
			       vif->wdev.address, ETH_ALEN);
			all_param_req->sub_msg_id[all_param_req->sub_msg_cnt] =
				MM_ADD_IF_REQ;
		...
		error = rwnx_send_msg(rwnx_hw, all_param_req, 0,
				      MM_SET_RESUME_RESTORE_CFM, NULL);

		return error;
	}

	int rwnx_prepare_resume_restore_req(struct rwnx_hw * rwnx_hw,
					    lmac_msg_id_t const id, void *param,
					    u16_l const param_len)
	{
		bool found = false;
		u32_l i = 0;
		...
		if (id == MM_SET_VENDOR_SWCONFIG_REQ) {
			struct mm_set_vendor_swconfig_req *req =
				(struct mm_set_vendor_swconfig_req *)param;
			...
			case EXT_FLAGS_MASK_SET_REQ: {
				for (i = 0;
				     i < rwnx_hw->all_restore_param
						 .vendor_swconfig_req_cnt;
				     i++) {
					if (rwnx_hw->all_restore_param
						    .vendor_swconfig_req[i]
						    .swconfig_id ==
					    swconfig_id) {
						found = true;
						memcpy(&rwnx_hw->all_restore_param
								.vendor_swconfig_req
									[i],
						       req,
						       sizeof(struct mm_set_vendor_swconfig_req));
				...
				if (found == false) {
					rwnx_hw->all_restore_param.sub_msg_id
						[rwnx_hw->all_restore_param
							 .sub_msg_cnt] = id;
					rwnx_hw->all_restore_param.sub_msg_cnt++;
					memcpy(&rwnx_hw->all_restore_param.vendor_swconfig_req
							[rwnx_hw->all_restore_param
								 .vendor_swconfig_req_cnt],
					       req,
					       sizeof(struct mm_set_vendor_swconfig_req));
			...
			for (i = 0; i < rwnx_hw->all_restore_param.sub_msg_cnt;
			     i++) {
				if (rwnx_hw->all_restore_param.sub_msg_id[i] ==
				    id) {
					found = true;
					break;
				}
			...
			if (found == false) {
				rwnx_hw->all_restore_param
					.sub_msg_id[rwnx_hw->all_restore_param
							    .sub_msg_cnt] = id;
				rwnx_hw->all_restore_param.sub_msg_cnt++;
			}
			switch (id) {
			...
			case MM_SET_TXPWR_IDX_LVL_REQ:
				memcpy(&rwnx_hw->all_restore_param.txpwr_lvl_req,
				       param, param_len);
				break;
			...
			case MM_SET_RF_CALIB_REQ:
				memcpy(&rwnx_hw->all_restore_param.rf_calib_req,
				       param, param_len);
				break;
			case MM_RESET_REQ:
				break;
			case ME_CONFIG_REQ:
				memcpy(&rwnx_hw->all_restore_param.config_req,
				       param, param_len);
				break;
			...
			case MM_SET_COEX_REQ:
				memcpy(&rwnx_hw->all_restore_param.coex_req,
				       param, param_len);
				break;
			case MM_START_REQ:
				memcpy(&rwnx_hw->all_restore_param.start_req,
				       param, param_len);
				break;
			case DBG_PWM_INIT_REQ:
				memcpy(&rwnx_hw->all_restore_param.pwm_init_req,
				       param, param_len);
				break;
			default:
				break;
	...
	int rwnx_send_wifi_disable_req(struct rwnx_hw * rwnx_hw,
				       struct dbg_wifi_disable_cfm * cfm)
	{
		...
		req = rwnx_msg_zalloc(MM_GET_WIFI_DISABLE_REQ, TASK_MM,
				      DRV_TASK_ID, sizeof(u8));
		if (!req)
			return -ENOMEM;

		ret = rwnx_send_msg(rwnx_hw, req, 1, MM_GET_WIFI_DISABLE_CFM,
				    cfm);
