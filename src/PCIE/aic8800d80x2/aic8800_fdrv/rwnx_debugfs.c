...
static ssize_t rwnx_dbgfs_debug_read(struct file *file, char __user *user_buf,
				     size_t count, loff_t *ppos)
{
						...
						size_t count, loff_t *ppos)
{
	...
	if (ret > 3) {
		...
		case 6: // TWO_ANT_RSSI_GET_REQ
			ret = rwnx_send_vendor_swconfig_req(priv, swconfig_id,
							    addr, addr_out);
			printk("2 ant rssi=%x\n", addr_out[0]);
			break;
		default:
...
static void idx_to_rate_cfg1(unsigned int formatmod, unsigned int mcs,
			     ...
			     unsigned int gi, unsigned int dcm,
						  ...
						  size_t count, loff_t *ppos)
{
	...
	buf[len] = '\0';
	//sscanf(buf, "%i\n", &fixed_rate_idx);
	sscanf(buf, "%u %u %u %u %u %u", &formatmod, &mcs, &nss, &bwTx, &gi,
	       &dcm);
	printk("%u %u %u %u %u %u\n", formatmod, mcs, nss, bwTx, gi, dcm);

	if ((formatmod > 7) || (mcs > 11) || (nss > 8) || (bwTx > 6) ||
	    (gi > 3)) {
		printk("error parameter");
		return len;
	} else if ((dcm == 1 && mcs == 2) || (dcm == 1 && mcs > 4) ||
		   (dcm == 1 && formatmod < 5)) {
