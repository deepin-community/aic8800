...
extern void rwnx_rx_handle_msg(struct rwnx_hw *rwnx_hw,
...
void rwnx_task(unsigned long data)
{
	...
	if (rwnx_hw->pcidev->chip_id == PRODUCT_ID_AIC8800D80)
		status = *(volatile unsigned int
				   *)(rwnx_hw->pcidev->pci_bar1_vaddr +
				      AIC8800D80_PCIE_IRQ_STATUS_OFFSET);
	else
		status = readl(adev->emb_mbox +
			       AIC8800D80X2_PCIE_IRQ_STATUS_OFFSET);
	...
	while (status) {
		...
		if (rwnx_hw->pcidev->chip_id == PRODUCT_ID_AIC8800D80) {
			volatile unsigned int *ack =
				(volatile unsigned int
					 *)(rwnx_hw->pcidev->pci_bar1_vaddr +
					    AIC8800D80_PCIE_IRQ_ACK_OFFSET);
			ack[0] = status;
		} else
			writel(status,
			       adev->emb_mbox +
				       AIC8800D80X2_PCIE_IRQ_ACK_OFFSET);

		if ((status & PCIE_TXC_DATA_BIT) && !txdata_pause) {
			fw_txc = *(volatile uint32_t *)&rwnx_hw->ipc_env->shared
					  ->txc_cnt;
		...
		if ((status & PCIE_RX_DATA_BIT) && !rxdata_pause) {
			fw_rxc = *(volatile uint32_t *)&rwnx_hw->ipc_env->shared
					  ->rxc_cnt;
		...
		if (status & PCIE_RX_MSG_BIT) {
			struct rwnx_ipc_buf *buf =
				rwnx_hw->ipc_env
					->msgbuf[rwnx_hw->ipc_env->msgbuf_idx];
			struct ipc_e2a_msg *msg = buf->addr;
			u8 ret = 0;

			while (1) {
				dma_sync_single_for_cpu(
					&rwnx_hw->pcidev->pci_dev->dev,
					buf->dma_addr,
					sizeof(struct ipc_e2a_msg),
					DMA_FROM_DEVICE);
				if (!msg ||
				    (msg->pattern != IPC_MSGE2A_VALID_PATTERN &&
				     msg->pattern != IPC_CFME2A_VALID_PATTERN &&
				     msg->pattern !=
					     IPC_PRTE2A_VALID_PATTERN)) {
				...
				if (msg->pattern != IPC_MSGE2A_VALID_PATTERN &&
				    msg->pattern != IPC_CFME2A_VALID_PATTERN &&
				    msg->pattern != IPC_PRTE2A_VALID_PATTERN) {
					ret = -1;
					//goto msg_no_push;
					AICWFDBG(LOGERROR,
						 "pattern error: 0x%x\n",
				...
				if (msg->pattern == IPC_MSGE2A_VALID_PATTERN)
					rwnx_rx_handle_msg(rwnx_hw, msg);
					 ...
					 IPC_CFME2A_VALID_PATTERN) {
					uint32_t used_idx = msg->param[1];
					uint64_t host_id =
						rwnx_hw->ipc_env->tx_host_id
							[0][used_idx %
							    PCIE_TXDESC_CNT];
					struct sk_buff *skb =
						(struct sk_buff *)(uint64_t)
							host_id;
					...
					if ((struct rwnx_sw_txhdr *)rwnx_hw
						    ->ipc_env->txcfm[idx] !=
					    NULL) {
						AICWFDBG(LOGINFO, "not txc\n");
						txhdr->sw_hdr->cfmd = 1;
						aicwf_pcie_host_tx_cfm_handler(
							rwnx_hw->ipc_env,
							msg->param, 0);
					} else
						aicwf_pcie_host_tx_cfm_handler(
		...
		if ((status & PCIE_RX_DATA_BIT) || rxdata_pause) {
			...
			volatile struct ipc_shared_rx_buf *rxbuf =
				&rwnx_hw->ipc_env->shared->host_rxbuf[data_cnt];
			...
			while (1) {
				...
				if (rxc == 0 &&
				    pattern != PCIE_RXDATA_COMP_PATTERN) {
					break;
				}

				if (rxdata_successive_cnt >= 10) {
					if (rwnx_hw->pcidev->chip_id ==
				...
				dma_sync_single_for_cpu(
					&rwnx_hw->pcidev->pci_dev->dev,
					ipc_buf->dma_addr, 2048,
					DMA_FROM_DEVICE);
#else
				dma_sync_single_for_cpu(
					&rwnx_hw->pcidev->pci_dev->dev,
					ipc_buf->dma_addr, 8192,
					DMA_FROM_DEVICE);
#endif
				...
				//printk("enq:%d,%p\n",data_cnt, skb);
				spin_lock_bh(
					&rwnx_hw->pcidev->rx_priv->rxqlock);
				aicwf_rxframe_enqueue(
					&rwnx_hw->pcidev->rx_priv->rxq, skb);
				atomic_inc(&rwnx_hw->pcidev->rx_priv->rx_cnt);
#ifdef CONFIG_RX_TASKLET
				if ((atomic_read(&rwnx_hw->pcidev->rx_priv
							  ->rx_cnt) == 1 ||
				     atomic_read(&rwnx_hw->pcidev->rx_priv
							  ->rx_wait) == 1))
					tasklet_schedule(
						&rwnx_hw->task_rx_process);
#else
				if ((atomic_read(&rwnx_hw->pcidev->rx_priv
							  ->rx_cnt) == 1 ||
				     atomic_read(&rwnx_hw->pcidev->rx_priv
							  ->rx_wait) == 1))
					complete(&rwnx_hw->pcidev->bus_if
							  ->rx_trgg);
#endif
				spin_unlock_bh(
					&rwnx_hw->pcidev->rx_priv->rxqlock);
				...
				rxbuf = &rwnx_hw->ipc_env->shared
						 ->host_rxbuf[data_cnt];
				if (rxbuf) {
					if (rxc == 0) {
						fw_rxc = *(volatile uint32_t
								   *)&rwnx_hw
								  ->ipc_env
								  ->shared
								  ->rxc_cnt;
			...
			for (; atomic_read(&rwnx_hw->rxbuf_cnt) <
				...
				if (rwnx_ipc_rxbuf_alloc(rwnx_hw)) {
					AICWFDBG(
						LOGERROR,
						"rxbuf alloc fail,now rxbuf_cnt = %d \n",
						atomic_read(
							&rwnx_hw->rxbuf_cnt));
					if (atomic_read(&rwnx_hw->rxbuf_cnt) <
					    254)
						complete(
							&rwnx_hw->pcidev->bus_if
								 ->busrx_trgg);
					break;
				}
			}
		...
		if ((status & PCIE_TXC_DATA_BIT) || txdata_pause) {
			...
			struct rwnx_sw_txhdr *sw_txhdr =
				(struct rwnx_sw_txhdr *)
					rwnx_hw->ipc_env->txcfm[txcfm_idx];
			volatile struct txdesc_host *txdesc =
				&rwnx_hw->ipc_env->shared->txdesc[txcfm_idx];
...
//printk("txc\n");
					...
					rwnx_ipc_buf_a2e_release(rwnx_hw,
								 txcfm_buf);
					dma_unmap_single(
						rwnx_hw->dev,
						sw_txhdr->ipc_hostdesc.dma_addr,
						sw_txhdr->ipc_hostdesc.size,
						DMA_TO_DEVICE);
#ifdef CONFIG_CACHE_GUARD
					kmem_cache_free(rwnx_hw->sw_txhdr_cache,
							sw_txhdr);
#else
					kfree(sw_txhdr);
#endif
				...
				sw_txhdr = (struct rwnx_sw_txhdr *)rwnx_hw
						   ->ipc_env->txcfm[txcfm_idx];
				if (sw_txhdr != NULL) {
					txdesc = &rwnx_hw->ipc_env->shared
							  ->txdesc[txcfm_idx];
					if (txc == 0) {
						fw_txc = *(volatile uint32_t
								   *)&rwnx_hw
								  ->ipc_env
								  ->shared
								  ->txc_cnt;
