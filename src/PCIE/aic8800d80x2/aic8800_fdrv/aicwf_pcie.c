...
void aicwf_temp_ctrl(struct aic_pci_dev *pcidev)
{
	if (pcidev->set_level) {
		if (pcidev->set_level == 1) {
			pcidev->get_level = 1;
			aicwf_netif_ctrl(
				pcidev, pcidev->interval_t1 /*TMR_INTERVAL_1*/);
			//mdelay(1);
		} else if (pcidev->set_level == 2) {
			pcidev->get_level = 2;
			aicwf_netif_ctrl(
				pcidev, pcidev->interval_t2 /*TMR_INTERVAL_2*/);
			//mdelay(2);
		}
		return;
	} else {
		if (pcidev->cur_temp > (pcidev->tp_thd_1 - 8)) {
			...
			if (update_state(pcidev->cur_temp, pcidev->cur_stat) ==
			    1) {
				pcidev->get_level = 1;
				pcidev->cur_stat = 1;
				aicwf_netif_ctrl(
					pcidev,
					pcidev->interval_t1 /*TMR_INTERVAL_1*/);
				//mdelay(1);
				//break;
				//} else if ((sdiodev->cur_temp > TEMP_THD_2) || (sdiodev->cur_stat == 2)) {
			} else if (update_state(pcidev->cur_temp,
						pcidev->cur_stat) == 2) {
				pcidev->get_level = 2;
				pcidev->cur_stat = 2;
				aicwf_netif_ctrl(
					pcidev,
					pcidev->interval_t2 /*TMR_INTERVAL_2*/);
				//mdelay(2);
				//break;
			}
			return;
...
void aicwf_netif_worker(struct work_struct *work)
{
	struct aic_pci_dev *pcidev =
		container_of(work, struct aic_pci_dev, netif_work);
	unsigned long flags;
	struct rwnx_vif *rwnx_vif;
	spin_lock_irqsave(&pcidev->tx_flow_lock, flags);
	list_for_each_entry(rwnx_vif, &pcidev->rwnx_hw->vifs, list) {
...
void aicwf_temp_ctrl_worker(struct work_struct *work)
{
	...
	struct aic_pci_dev *pcidev =
		container_of(work, struct aic_pci_dev, tp_ctrl_work);
	...
	spin_lock_bh(&pcidev->tm_lock);
	if (pcidev->tm_start) {
		mod_timer(&pcidev->tp_ctrl_timer,
...
extern u8 data_cnt;
extern int rwnx_plat_bin_fw_upload_2(struct rwnx_hw *rwnx_hw, u32 fw_addr,
				     char *filename);
...
static int aicwf_sw_resume(void)
{
	...
	for (i = 0; i < IPC_RXBUF_CNT; i++) {
		...
		rwnx_hw->ipc_env->shared->host_rxbuf[i].hostid =
			RWNX_RXBUFF_HOSTID_GET(ipc_buf);
...
#ifdef USE_5G
	ret = rwnx_send_resume_restore(rwnx_hw, 1, 0, CO_BIT(5), 0,
				       rwnx_vif_param);
#else
	ret = rwnx_send_resume_restore(rwnx_hw, 1, feature.hwinfo < 0,
				       feature.hwinfo, 0, rwnx_vif_param);
#endif
	if (ret) {
...
static int aicwf_disconnect_inform(struct rwnx_hw *rwnx_hw,
				   struct rwnx_vif *rwnx_vif)
{
...
static int aicwf_pcie_init(struct aic_pci_dev *pciedev)
{
	...
	case PRODUCT_ID_AIC8800D80X2:
		...
		LOG_INFO("bar0: %x, len = %x, map = %lx", adev->bar0,
			 adev->len0, (unsigned long)adev->map0);
	...
	ret = request_irq(pci_dev->irq, aicwf_pcie_irq_hdlr, IRQF_SHARED,
			  "aicwf_pci", pciedev);
	if (ret) {
	...
	if ((pciedev->chip_id == PRODUCT_ID_AIC8800D80) ||
		...
		pci_read_config_byte(
			pci_dev, pci_dev->pcie_cap + PCI_EXP_LNKCTL, &linkctrl);
		if (linkctrl & 0x02) {
			linkctrl = linkctrl & ~0x02;
			pci_write_config_byte(
				pci_dev, pci_dev->pcie_cap + PCI_EXP_LNKCTL,
				linkctrl);
		}
	}
...
static void aicwf_pcie_txmsg_db(struct aic_pci_dev *pciedev)
{
	if (pciedev->chip_id == PRODUCT_ID_AIC8800D80) {
		volatile unsigned int *dst_mail =
			(volatile unsigned int *)(pciedev->pci_bar2_vaddr +
						  0x800ec);
		dst_mail[0] = 0x1;
	} else
...
static int aicwf_pcie_probe(struct pci_dev *pci_dev,
...
static void aicwf_pcie_remove(struct pci_dev *pci_dev)
{
	...
	struct ipc_shared_env_tag *shared =
		(struct ipc_shared_env_tag *)(pci->emb_shrm);
...
static int aicwf_pcie_suspend(struct pci_dev *pdev, pm_message_t state)
{
	...
	list_for_each_entry(rwnx_vif, &rwnx_hw->vifs, list) {
		if (rwnx_vif->up) {
			while ((int)atomic_read(&rwnx_vif->drv_conn_state) ==
			       (int)RWNX_DRV_STATUS_DISCONNECTING) {
				LOG_ERROR("suspend waiting disc\n");
				msleep(100);
				i += 1;
				if (i >= 20) {
					aicwf_disconnect_inform(rwnx_hw,
								rwnx_vif);
					break;
				}
			}
...
static int aicwf_pcie_resume(struct pci_dev *pdev)
{
	...
	if (g_rwnx_plat->pcidev->chip_id == PRODUCT_ID_AIC8800D80) {
		fw_started =
			*(volatile u32 *)(g_rwnx_plat->pcidev->pci_bar0_vaddr +
					  0x120000) == 0x1a2000;
	} else {
...
#ifdef CONFIG_TEMP_CONTROL
		mod_timer(&g_rwnx_plat->pcidev->tp_ctrl_timer,
			  jiffies + msecs_to_jiffies(TEMP_GET_INTERVAL));
#endif
...
int pcie_rxbuf_rep_thread(void *data)
{
	...
	while (1) {
		...
		if (!wait_for_completion_interruptible(&bus_if->busrx_trgg)) {
			...
			if (g_rwnx_plat->pcidev &&
			    g_rwnx_plat->pcidev->rwnx_hw) {
				rwnx_hw = g_rwnx_plat->pcidev->rwnx_hw;
			} else {
			...
			for (; atomic_read(&rwnx_hw->rxbuf_cnt) <
			       rwnx_hw->ipc_env->rxbuf_nb;) {
				if (bus_if->state == BUS_DOWN_ST) {
					break;
				}
				if (rwnx_ipc_rxbuf_alloc(rwnx_hw)) {
					AICWFDBG(
						LOGERROR,
						"pcie_rxbuf_rep_thread rxbuf alloc fail,now rxbuf_cnt = %d \n",
						atomic_read(
							&rwnx_hw->rxbuf_cnt));
					msleep(10);
					//break;
	...
	while (1) {
		...
		if (hw_rxhdr->flags_is_amsdu) {
			...
			for (i = 0; i < hw_rxhdr->hwvect.reserved - 1; i++) {
				spin_lock_bh(&rx_priv->rxqlock);
				while (aicwf_is_framequeue_empty(
					&rx_priv->rxq)) {
			...
			if (mpdu_len > 2048) {
				...
				if (hw_rxhdr->hwvect.reserved > 1) {
					...
					for (i = 0;
					     i < hw_rxhdr->hwvect.reserved - 1;
					     i++) {
						int copy_len = mpdu_len > 2048 ?
								       2048 :
								       mpdu_len;
						if (skb_new)
							memcpy(skb_new->data +
		...
		if (!wait_for_completion_interruptible(&bus_if->rx_trgg)) {
			...
			while (1) {
				...
				if (hw_rxhdr->hwvect.reserved >
				    (rx_priv->rxq.qcnt)) {
					//printk("rx wait:%d,%d\n", hw_rxhdr->hwvect.reserved,rx_priv->rxq.qcnt);
					atomic_set(&rwnx_hw->pcidev->rx_priv
							    ->rx_wait,
						   1);
					spin_unlock_bh(&rx_priv->rxqlock);
					break;
				...
				if (atomic_read(&rwnx_hw->pcidev->rx_priv
							 ->rx_wait) == 1)
					atomic_set(&rwnx_hw->pcidev->rx_priv
							    ->rx_wait,
						   0);
...
int aicwf_pcie_bus_init(struct aic_pci_dev *pciedev)
{
	...
	mod_timer(&pciedev->tp_ctrl_timer,
		  jiffies + msecs_to_jiffies(TEMP_GET_INTERVAL));
