...
struct rwnx_mod_params rwnx_mod_params = {
								...
								COMMON_PARAM(
									...
									COMMON_PARAM(
										...
										COMMON_PARAM(
											amsdu_maxnb,
											4,
											4) //NX_TX_PAYLOAD_MAX, NX_TX_PAYLOAD_MAX)
	...
	COMMON_PARAM(uapsd_queues, IEEE80211_WMM_IE_STA_QOSINFO_AC_VO,
		...
		COMMON_PARAM(tdls, false, false) COMMON_PARAM(uf, false, false)
			COMMON_PARAM(ftl, "", "") COMMON_PARAM(
				dpsm, false, false) COMMON_PARAM(tx_to_bk, 0, 0)
				COMMON_PARAM(tx_to_be, 0,
					     0) COMMON_PARAM(tx_to_vi, 0, 0)
...
MODULE_PARM_DESC(amsdu_tx,
		 "Use A-MSDU in TX: 0-if advertised, 1-yes, 2-no (Default: 0)");
...
static void rwnx_set_vht_capa(struct rwnx_hw *rwnx_hw, struct wiphy *wiphy)
{
	...
	if (rwnx_hw->pcidev->chip_id == PRODUCT_ID_AIC8800D80X2 &&
	    rwnx_hw->mod_params->stbc_on == true)
...
static void rwnx_set_wiphy_params(struct rwnx_hw *rwnx_hw, struct wiphy *wiphy)
{
	...
	(LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0))
		...
		regdomain = getRegdomainFromRwnxDB(wiphy, default_ccode);
		AICWFDBG(LOGINFO,
			 KERN_CRIT "%s: Registering custom regulatory\n",
			 __func__);
