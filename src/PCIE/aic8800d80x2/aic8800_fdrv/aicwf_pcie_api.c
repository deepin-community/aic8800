...
void aicwf_pcie_cfg(struct aic_pci_dev *adev)
{
	...
	LOG_INFO("host dma");
	aic_pcie_map_set(adev, 0, AIC_MAP_HDMA_BASE, AIC_MAP_HDMA_LIMT,
			 AIC_MAP_HDMA_ADDR);
	LOG_INFO("top pcie");
	aic_pcie_map_set(adev, 1, AIC_MAP_TPCI_BASE, AIC_MAP_TPCI_LIMT,
			 AIC_MAP_TPCI_ADDR);
	LOG_INFO("mail box");
	aic_pcie_map_set(adev, 2, AIC_MAP_MBOX_BASE, AIC_MAP_MBOX_LIMT,
			 AIC_MAP_MBOX_ADDR);
	LOG_INFO("cpu sys");
	aic_pcie_map_set(adev, 3, AIC_MAP_SCTL_BASE, AIC_MAP_SCTL_LIMT,
			 AIC_MAP_SCTL_ADDR);
	LOG_INFO("share mem");
	aic_pcie_map_set(adev, 4, AIC_MAP_SHRM_BASE, AIC_MAP_SHRM_LIMT,
			 AIC_MAP_SHRM_ADDR);
...
void aicwf_pcie_print_st(struct aic_pci_dev *adev, u32 sim)
{
	...
	if (sim == 0) {
		...
		LOG_INFO(" %x = %x\n", addr, rd);
		for (k = 0; k <= 4; k++) {
			pci_write_config_dword(adev->pdev, 0x900,
					       0x80000000 + k);
			for (n = 0; n <= 0x7; n++) {
				addr = 0x900 + (n << 2);
				pci_read_config_dword(adev->pdev, addr, &x[n]);
			}
