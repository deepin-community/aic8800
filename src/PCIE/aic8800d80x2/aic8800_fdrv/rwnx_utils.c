...
void aic_host_tx_flush(struct rwnx_hw *rwnx_hw)
{
	...
	while (idx < IPC_TXDMA_DESC_CNT) {
		...
		AICWFDBG(
			LOGDEBUG,
			"aic_host_tx_flush: Completed TX descriptor found at index %d\n"
		...
		if (skb_tmp) {
			AICWFDBG(
				LOGDEBUG,
				"aic_host_tx_flush: Consuming skb at %p with headroom %ld\n",
	...
	for (queue_idx = 0; queue_idx < PCIE_TXQUEUE_CNT; queue_idx++) {
		for (desc_idx = 0; desc_idx < PCIE_TXDESC_CNT; desc_idx++) {
			if (rwnx_hw->ipc_env->tx_host_id[queue_idx][desc_idx]) {
				AICWFDBG(
					LOGTRACE,
					"aic_host_tx_flush: Found non-zero entry in tx_host_id[%d][%d]\n",
					queue_idx, desc_idx);
				struct sk_buff *skb_tmp =
					(struct sk_buff *)(uint64_t)rwnx_hw
						->ipc_env->tx_host_id[queue_idx]
								     [desc_idx];
				sw_txhdr = ((struct rwnx_txhdr *)skb_tmp->data)
						   ->sw_hdr;
				...
				if (skb_tmp) {
					AICWFDBG(
						LOGDEBUG,
						"aic_host_tx_flush: Consuming skb at %p with headroom %d\n",
						skb_tmp, headroom);
					skb_pull(skb_tmp, headroom);
					consume_skb(skb_tmp);
				}
				rwnx_hw->ipc_env
					->tx_host_id[queue_idx][desc_idx] = 0;
			}
		}
	}
}
...
void rwnx_ipc_tx_drain(struct rwnx_hw *rwnx_hw)
{
	...
	if (!rwnx_hw->ipc_env) {
		AICWFDBG(LOGERROR,
			 KERN_CRIT "%s: bypassing (restart must have failed)\n",
			 __func__);
		return;
	}
...
void rwnx_pcie_shared_init(struct rwnx_hw *rwnx_hw)
{
	...
	if (rwnx_hw->pcidev->chip_id == PRODUCT_ID_AIC8800D80) {
		aicwf_pcie_host_init(
			...
			(struct ipc_shared_env_tag
				 *)(rwnx_hw->pcidev->pci_bar0_vaddr + 0x1DC000),
			rwnx_hw);
		shared_env =
			(struct ipc_shared_env_tag
				 *)(rwnx_hw->pcidev->pci_bar0_vaddr + 0x1DC000);
	...
	AICWFDBG(
		LOGINFO,
		"sizeof struct ipc_shared_env_tag is %ld byte,  offset=%ld, %ld, %ld , txdesc %lx\n",
