...
void aic_pcie_map_set(struct aic_pci_dev *adev, u8 idx, u32 base, u32 limt,
		      u32 addr)
{
	u32 bar0;
...
void aicwf_pcie_cfg(struct aic_pci_dev *adev)
{
	...
	if (adev->chip_id == PRODUCT_ID_AIC8800D80X2) {
		pcie_capability_clear_word(adev->pdev, PCI_EXP_LNKCTL,
					   PCI_EXP_LNKCTL_CLKREQ_EN);
		pcie_capability_clear_word(adev->pdev, PCI_L1SS_CTL1,
	...
	if (adev->chip_id == PRODUCT_ID_AIC8800D80) {
		LOG_INFO("host dma");
		aic_pcie_map_set(adev, 0, AIC_MAP_HDMA_BASE_AIC8800D80,
				 AIC_MAP_HDMA_LIMT_AIC8800D80,
				 AIC_MAP_HDMA_ADDR_AIC8800D80);
		LOG_INFO("top pcie");
		aic_pcie_map_set(adev, 1, AIC_MAP_TPCI_BASE_AIC8800D80,
				 AIC_MAP_TPCI_LIMT_AIC8800D80,
				 AIC_MAP_TPCI_ADDR_AIC8800D80);
		LOG_INFO("mail box");
		aic_pcie_map_set(adev, 2, AIC_MAP_MBOX_BASE_AIC8800D80,
				 AIC_MAP_MBOX_LIMT_AIC8800D80,
				 AIC_MAP_MBOX_ADDR_AIC8800D80);
		LOG_INFO("cpu sys");
		aic_pcie_map_set(adev, 3, AIC_MAP_SCTL_BASE_AIC8800D80,
				 AIC_MAP_SCTL_LIMT_AIC8800D80,
				 AIC_MAP_SCTL_ADDR_AIC8800D80);
		LOG_INFO("share mem");
		aic_pcie_map_set(adev, 4, AIC_MAP_SHRM_BASE_AIC8800D80,
				 AIC_MAP_SHRM_LIMT_AIC8800D80,
				 AIC_MAP_SHRM_ADDR_AIC8800D80);
		...
		LOG_INFO("host dma");
		aic_pcie_map_set(adev, 0, AIC_MAP_HDMA_BASE_AIC8800D80X2,
				 AIC_MAP_HDMA_LIMT_AIC8800D80X2,
				 AIC_MAP_HDMA_ADDR_AIC8800D80X2);
		LOG_INFO("top pcie");
		aic_pcie_map_set(adev, 1, AIC_MAP_TPCI_BASE_AIC8800D80X2,
				 AIC_MAP_TPCI_LIMT_AIC8800D80X2,
				 AIC_MAP_TPCI_ADDR_AIC8800D80X2);
		LOG_INFO("mail box");
		aic_pcie_map_set(adev, 2, AIC_MAP_MBOX_BASE_AIC8800D80X2,
				 AIC_MAP_MBOX_LIMT_AIC8800D80X2,
				 AIC_MAP_MBOX_ADDR_AIC8800D80X2);
		LOG_INFO("cpu sys");
		aic_pcie_map_set(adev, 3, AIC_MAP_SCTL_BASE_AIC8800D80X2,
				 AIC_MAP_SCTL_LIMT_AIC8800D80X2,
				 AIC_MAP_SCTL_ADDR_AIC8800D80X2);
		LOG_INFO("share mem");
		aic_pcie_map_set(adev, 4, AIC_MAP_SHRM_BASE_AIC8800D80X2,
				 AIC_MAP_SHRM_LIMT_AIC8800D80X2,
				 AIC_MAP_SHRM_ADDR_AIC8800D80X2);
...
int aicwf_pcie_dma(struct aic_pci_dev *adev, void *dst, void *src, u32 size,
		   u8 dir)
{
	//> aic pcie dma api
	...
	LOG_INFO("    aic dma set: %08llx -> %08llx, size = %x", (u64)src,
		 (u64)dst, size);
	...
	while (size > aic_dma_mps) {
		...
		adma->next =
			aic_map_shrm_addr + aic_map_shrm_dma_off + num * 16;
	...
	root_off = (dir == AIC_TRAN_DRV2EMB) ? 0x08 :
					       0x00; // lli_root2 : lli_root0
	lli_off = 0xBC; // lli_counter15
	lli_old = readl(adev->emb_hdma + lli_off);
	lli_tar = lli_old + num;
	wmb();
	writel(aic_map_shrm_addr + aic_map_shrm_dma_off,
	       adev->emb_hdma + root_off);
	wmb();
	...
	if (rd == lli_tar) {
		LOG_INFO(
			"    aic dma done, dma irq success, lli_cnt %d -> %d(%d)",
			lli_old, rd, lli_tar);
		return 0;
	} else {
		LOG_WARN(
			"    aic dma done, dma irq failed!, lli_cnt %d -> %d(%d)",
			lli_old, rd, lli_tar);
		return -1;
	}
}

int aicwf_pcie_tran(struct aic_pci_dev *adev, void *addr_emb, void *buf,
		    u32 blen, u8 dir, u8 rem)
{
	...
	if (dir == AIC_TRAN_DRV2EMB) {
		...
		ret = aicwf_pcie_dma(adev, addr_emb, (void *)addr_dma, blen,
				     dir);
	} else {
		ret = aicwf_pcie_dma(adev, (void *)addr_dma, addr_emb, blen,
				     dir);
#ifdef AIC_PCIE_API_DELAY
...
int aicwf_pcie_test_dma(struct aic_pci_dev *adev, u32 off, u16 wlen, u8 dir)
{
	...
	LOG_INFO("***** TEST TRANSFER DATA %s *****",
		 (dir == AIC_TRAN_DRV2EMB) ? "drv2emb" : "emb2drv");
	...
	if (ret == 0) {
		for (k = 0; k < wlen; k++) {
			...
			xdut = (dir == AIC_TRAN_DRV2EMB) ?
				       readl(addr_emb_map + k * 4) :
				       ((u32 *)addr_cpu)[k];
			if (xref != xdut) {
				LOG_WARN("FAIL itran[%x], %x, %x", k, xref,
					 xdut);
				ret = -3;
				break;
			}
...
int aicwf_pcie_setst(struct aic_pci_dev *adev)
{
	...
	ret = aicwf_pcie_test_dma(adev, AIC_TEST_TRAN_OFF0, AIC_TEST_TRAN_WLEN,
				  AIC_TRAN_DRV2EMB);
	if (ret) {
		LOG_INFO("FAILED TEST: aic_pcie_test_accs_emb %d", ret);
		return ret;
	...
	ret = aicwf_pcie_test_dma(adev, AIC_TEST_TRAN_OFF1, AIC_TEST_TRAN_WLEN,
				  AIC_TRAN_EMB2DRV);
	if (ret) {
		LOG_INFO("FAILED TEST: aic_pcie_test_accs_emb %d", ret);
		return ret;
