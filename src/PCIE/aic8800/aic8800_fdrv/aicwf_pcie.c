...
static int aicwf_pcie_init(struct aic_pci_dev *pciedev)
{
	...
	case PRODUCT_ID_AIC8800D80:
		if (pciedev->bar_count == 1) {
			...
			adev->len0 = pci_resource_len(adev->pdev, 0);
			if (!(adev->map0 =
				      (u8 *)pci_ioremap_bar(adev->pdev, 0))) {
				dev_err(&(pci_dev->dev),
					"pci_ioremap_bar0 failed\n");
				ret = -ENODEV;
				goto out_bar0;
			}
#endif
		...
		if (pciedev->bar_count == 1) {
			...
			volatile unsigned int *dst_mail =
				(volatile unsigned int
					 *)(pciedev->pci_bar2_vaddr + 0x800ec);
			dst_mail[0] = 0x1;
		}
	} else {
		writel(1, pciedev->emb_tpci + 0x0ec);
...
static void aicwf_pcie_remove(struct pci_dev *pci_dev)
{
	...
	struct ipc_shared_env_tag *shared =
		(struct ipc_shared_env_tag *)(pci->pci_bar0_vaddr + 0x1DC000);
	*(volatile uint32_t *)&shared->fw_init_done = 0;
	//writel(4, pci->emb_tpci + 0x0ec); //generate an empty int
	volatile unsigned int *dst_mail =
		(volatile unsigned int *)(pci->pci_bar2_vaddr + 0x800ec);
...
static int aicwf_pcie_resume(struct pci_dev *pdev)
{
	...
	fw_started = *(volatile u32 *)(g_rwnx_plat->pcidev->pci_bar0_vaddr +
				       0x120000) == 0x1a2000;
