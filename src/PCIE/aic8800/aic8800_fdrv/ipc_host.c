...
static void ipc_host_radar_handler(struct ipc_host_env_tag *env)
{
	...
	while (env->cb.recv_radar_ind(env->pthis, env->radar[env->radar_idx]) ==
	       0)
		;
...
static void ipc_host_unsup_rx_vec_handler(struct ipc_host_env_tag *env)
{
	while (env->cb.recv_unsup_rx_vec_ind(
		       env->pthis, env->unsuprxvec[env->unsuprxvec_idx]) == 0)
		;
}
...
{
	while (env->cb.recv_msg_ind(env->pthis, env->msgbuf[env->msgbuf_idx]) ==
	       0)
		;
}
...
{
	while (env->cb.recv_dbg_ind(env->pthis, env->dbgbuf[env->dbgbuf_idx]) ==
	       0)
		;
}
 ...
 ******************************************************************************
 */
void ipc_host_pattern_push(struct ipc_host_env_tag *env,
 ...
 ******************************************************************************
 */
int ipc_host_unsuprxvec_push(struct ipc_host_env_tag *env,
			     struct rwnx_ipc_buf *buf)
{
	struct ipc_shared_env_tag *shared_env_ptr = env->shared;

	env->unsuprxvec[env->unsuprxvec_idx] = buf;
	shared_env_ptr->unsuprxvecbuf_hostbuf[env->unsuprxvec_idx] =
		buf->dma_addr;
 ...
 ******************************************************************************
 */
void ipc_host_dbginfo_push(struct ipc_host_env_tag *env,
			   struct rwnx_ipc_buf *buf)
{
	struct ipc_shared_env_tag *shared_env = env->shared;
 ...
 */
uint32_t ipc_host_tx_host_ptr_to_id(struct ipc_host_env_tag *env,
				    void *host_ptr)
{
	struct ipc_hostid *tx_hostid;
