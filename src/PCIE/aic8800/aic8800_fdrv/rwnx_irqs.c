...
void rwnx_task(unsigned long data)
{
	...
	while (status) {
		...
		if ((status & PCIE_RX_DATA_BIT) || rxdata_pause) {
			...
			while (1) {
				...
				if (mpdu_len > 2048 && mpdu_cnt == 0) {
					...
					copy_len = mpdu_len > 2048 ? 2048 :
								     mpdu_len;
					if (skb_new)
						memcpy(skb_new->data + offset,
						       skb->data, copy_len);
					mpdu_len -= copy_len;
					mpdu_cnt--;
					offset += copy_len;
		...
		if (status & PCIE_TXC_DATA_BIT || txdata_pause) {
			...
			while ((sw_txhdr != NULL) &&
			       (ready == PCIE_TXDATA_COMP_PATTERN)) {
