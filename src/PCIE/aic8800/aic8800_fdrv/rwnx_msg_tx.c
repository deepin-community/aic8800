 ...
 *********************************************************************/
	...
	int rwnx_send_pwm_init_req(struct rwnx_hw * rwnx_hw, u8 pwm_gpidx,
				   u8 mode, u8 run, u32 tmr_cnt, u32 dty_cnt,
				   u32 step_val, u8 gpio_en, u8 gpio_dir,
				   u8 gpio_val)
	{
		struct dbg_pwm_init_req *pwm_init_req;

		pwm_init_req = rwnx_msg_zalloc(DBG_PWM_INIT_REQ, TASK_DBG,
					       DRV_TASK_ID,
		...
		rwnx_prepare_resume_restore_req(
			rwnx_hw, DBG_PWM_INIT_REQ, pwm_init_req,
			sizeof(struct dbg_pwm_init_req));

		return rwnx_send_msg(rwnx_hw, pwm_init_req, 1, DBG_PWM_INIT_CFM,
				     NULL);
	}

	int rwnx_send_pwm_deinit_req(struct rwnx_hw * rwnx_hw, u8 pwm_gpidx,
				     u8 gpio_en, u8 gpio_dir, u8 gpio_val)
	{
		struct dbg_pwm_deinit_req *pwm_deinit_req;

		pwm_deinit_req = rwnx_msg_zalloc(
			DBG_PWM_DEINIT_REQ, TASK_DBG, DRV_TASK_ID,
		...
		return rwnx_send_msg(rwnx_hw, pwm_deinit_req, 1,
				     DBG_PWM_DEINIT_CFM, NULL);
	}
