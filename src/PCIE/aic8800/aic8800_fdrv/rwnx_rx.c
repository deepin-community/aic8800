	...
	if (((RWNX_VIF_TYPE(rwnx_vif) == NL80211_IFTYPE_AP) ||
		...
		if (unlikely(is_multicast_ether_addr(eth->h_dest))) {
			...
			if (rxhdr->flags_dst_idx != RWNX_INVALID_STA) {
				struct rwnx_sta *sta =
					&rwnx_hw->sta_table[rxhdr->flags_dst_idx];
				if (sta->valid &&
...
u8 rwnx_unsup_rx_vec_ind(void *pthis, void *arg)
{
	...
	if (((struct rx_vector_desc *)(skb->data))->pattern == 0) {
		rwnx_ipc_buf_e2a_sync_back(rwnx_hw, ipc_buf,
					   sizeof(struct rx_vector_desc));
		return -1;
	}
	...
	/* Add RadioTap Header */
	rwnx_rx_add_rtap_hdr(rwnx_hw, skb, rx_vect1, phy_info, NULL, rtap_len,
			     vend_rtap_len, BIT(0));
...
u8 rwnx_rxdataind(void *pthis, void *arg)
{
	...
	if (!status) {
		...
		rwnx_ipc_buf_e2a_sync_back(rwnx_hw, ipc_desc,
...
#endif
{
	...
	struct reord_ctrl *preorder_ctrl =
		timer_container_of(preorder_ctrl, t, reord_timer);
#else
	struct reord_ctrl *preorder_ctrl =
...
#else
			rwnx_cfg80211_rx_spurious_frame(
				rwnx_vif->ndev, hdr->addr2, -1, GFP_ATOMIC);
#endif
		}
		goto end;
	}
